...
function mission() {
  let s = initial();
  for (const e of ['prologue', 'prologue', 'mission', 'start']) s = advance(s, e);
  return s;
}
function assembled() {
  let s = mission();
  for (const m of content.chapter.modules) s = advance(s, 'inspect', m.id);
  s = advance(s, 'assembleStart');
  for (const m of content.chapter.modules) s = advance(s, 'place', m.id);
  return advance(s, 'testStart');
}
test('完整路径：文化导入、拆解、组装、排障、创新、承诺后达到 100%', () => {
  let s = assembled();
  assert.equal(s.step, 3);
  s = advance(s, 'anomaly', 'b');
  s = advance(s, 'calibrate', 'verify');
  s = advance(s, 'innovationStart');
  s = advance(s, 'innovate', 'bidirectional');
...
test('不能跳过使命、模块认识、复测或行动承诺', () => {
  let s = initial();
  for (const e of ['mission', 'start', 'assembleStart', 'testStart', 'innovationStart', 'complete'])
    s = advance(s, e);
  ...
  s = advance(s, 'place', 'meter');
  assert.equal(s.assembled.length, 0);
  s = assembled();
  s = advance(s, 'calibrate', 'verify');
...
test('错误判断不解锁后续，正确选择可恢复', () => {
  let s = assembled();
  s = advance(s, 'anomaly', 'a');
  assert.equal(s.anomalyFound, false);
  assert.equal(s.mistakes, 1);
  s = advance(s, 'anomaly', 'b');
  s = advance(s, 'calibrate', 'display');
  assert.equal(s.calibrated, false);
  s = advance(s, 'calibrate', 'verify');
...
test('重复点选不累计模块；存档序列化后保留中途进度', () => {
  let s = mission();
  s = advance(s, 'inspect', 'meter');
  s = advance(s, 'inspect', 'meter');
  assert.deepEqual(s.inspected, ['meter']);
...
test('每个进度检查点恢复后的下一步保持可达', () => {
  ...
  const events = [
    ...
    ['mission'],
    ['start'],
    ...content.chapter.modules.map((m) => ['inspect', m.id]),
    ['assembleStart'],
    ...content.chapter.modules.map((m) => ['place', m.id]),
    ['testStart'],
    ['anomaly', 'b'],
    ['calibrate', 'verify'],
    ['innovationStart'],
    ['innovate', 'bidirectional'],
    ['complete', content.actions[1]]
  ];
  let previous = 0;
  for (const [e, p] of events) {
    s = normalize(JSON.parse(JSON.stringify(advance(s, e, p))));
...
test('第二、第三阶段的答题独立保存，错误答案不会点亮关卡', () => {
  ...
  assert.equal(s.missions['3'], false);
