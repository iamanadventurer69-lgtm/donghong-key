...
(function (App) {
  ...
  App.meter = function meter(options) {
    ...
    function reading(value) {
      return String(value * WATTS_PER_PERCENT);
    }

    function waveOffset(x) {
      if (!running) return 0;
      return (Math.sin((x / width) * Math.PI * 5 - phase) * height * 0.38 * load) / 100;
    }
    ...
    function draw() {
      ...
      ctx.strokeStyle = options.calibrated() ? '#249dde' : '#66bde9';
      ctx.lineWidth = 2;
      ctx.beginPath();
      for (let x = 0; x <= width; x += 2) {
        const y = height / 2 + waveOffset(x);
    ...
    function emit() {
      if (options.onChange)
        options.onChange({ load, running, power: running ? reading(load) : '0000' });
    }
    ...
    return {
      ...
      setLoad(value) {
        load = Math.max(LOAD_MIN, Math.min(LOAD_MAX, Number(value) || LOAD_MIN));
        emit();
        draw();
        return load;
      },
